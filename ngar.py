...
class NGAR():
    ...
    def UpdateTheta(self, it):
        ...
        if it <100:
            ...
            var_star_1 =(np.array([[self.sum_2_sigma_sq[0],self.sum_2_sigma_sq[1],self.sum_2_sigma_sq[3]],[self.sum_2_sigma_sq[1],self.sum_2_sigma_sq[2],self.sum_2_sigma_sq[4]],[self.sum_2_sigma_sq[3],self.sum_2_sigma_sq[4],self.sum_2_sigma_sq[5]]])- np.asarray(np.asmatrix(self.sum_1_sigma_sq).T*np.asmatrix(self.sum_1_sigma_sq)*1.0/it))*1.0/(it-1)
        ...
        for jj in range(self.p):
            if z_star[jj]== True:
                ...
                if it < self.start_adap:
                    ...
                    var_star_1 = 1.0*(sxx-np.asarray(np.asmatrix(self.sum_1[:,jj]).T*np.asmatrix(self.sum_1[:,jj]))*1.0/(it-self.start_samples))/(it-self.start_samples-1)
